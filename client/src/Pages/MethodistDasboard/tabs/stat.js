...
const StatTab = () => {
  ...
  useEffect(() => {
    ...
    if (sortField === 'contractDate') {
      sortedData.sort(
        (a, b) => new Date(a.contractDate) - new Date(b.contractDate)
      );
    } else if (sortField === 'contractPeriodStart') {
      sortedData.sort(
        (a, b) =>
          new Date(a.contractPeriodStart) - new Date(b.contractPeriodStart)
      );
    } else if (sortField === 'contractPeriodEnd') {
      sortedData.sort(
        (a, b) => new Date(a.contractPeriodEnd) - new Date(b.contractPeriodEnd)
      );
    } else if (sortField === 'addendumDate') {
      sortedData.sort(
        (a, b) => new Date(a.addendumDate) - new Date(b.addendumDate)
      );
    }
        ...
        <table className="stat-table">
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort('contractPeriodStart')}>
                Дата начала периода
              </th>
              <th onClick={() => handleSort('contractPeriodEnd')}>
                Дата окончания периода
              </th>
              <th>Номер дополнения</th>
              <th onClick={() => handleSort('addendumDate')}>
                Дата дополнения
              </th>
